...
const balance_sheet = {
    asset: async (from_date, to_date, account_type) => {
        const get_specific_account = await chartOfAccount_1.default.find({ account_type: account_type });
        const all_specific_account = get_specific_account.map((e) => e._id);
        const all_account_detail = await generalJournal_1.default.aggregate([
    ...
    total_asset_balance: async (from_date, to_date) => {
    ...
    liability: async (from_date, to_date, account_type) => {
        const get_specific_account = await chartOfAccount_1.default.find({ account_type: account_type });
        const all_specific_account = get_specific_account.map((e) => e._id);
        const all_account_detail = await generalJournal_1.default.aggregate([
            { $match: { transaction_date: { $gte: from_date, $lte: to_date } } },
    ...
    total_liability_balance: async (from_date, to_date) => {
    ...
    equity: async (from_date, to_date, account_type) => {
        const get_specific_account = await chartOfAccount_1.default.find({ account_type: account_type });
        const all_specific_account = get_specific_account.map((e) => e._id);
        const all_account_detail = await generalJournal_1.default.aggregate([
            { $match: { transaction_date: { $gte: from_date, $lte: to_date } } },
    ...
    total_equity_balance: async (from_date, to_date) => {
    ...
    total_liability_and_equity_balance: async (from_date, to_date) => {
